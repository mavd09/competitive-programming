...
int main() {
  ...
  while(cin >> n) {
    ...
    ll ans = 0;
    for(int i = 0; i < n; ++i) {
      int fx, tx, y; cin >> fx >> tx >> y;
      if(fx > tx) swap(fx, tx);
      all.push_back({fx, y});
      all.push_back({tx, y});
      segs.push_back({pii{fx, tx}, y});
      ans = max(ans, 1LL*(tx-fx));
    }
    for(auto& o : all) {
      vector<event> events;
      ll sum = 0;
      for(auto& seg : segs) {
        pt fr = pt{seg.first.first, seg.second} - o;
        pt to = pt{seg.first.second, seg.second} - o;
        int val = seg.first.second-seg.first.first;
        if(fr.y < 0) {
          fr = {-fr.x, -fr.y};
          to = {-to.x, -to.y};
          swap(fr, to);
        }
        if(fr.y == 0 && (fr.x <= 0 && 0 <= to.x)) sum += val;
        if(fr.y != 0) {
          // cout << val << endl;
          events.push_back({fr, -val});
          events.push_back({to, val});
        }
      }
      sort(events.begin(), events.end(), [&](const event& e1, const event& e2){
        return make_tuple(half(e1.p), 0, -e1.val) < make_tuple(half(e2.p), cross(e1.p,e2.p), -e2.val);
      });
      for(auto& e : events) {
        // cout << e.p << ' ' << e.val << endl;
        sum += e.val;
